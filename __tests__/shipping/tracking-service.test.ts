...
const MOCK_EVENTS: ShipmentTrackingEvent[] = [
  {
    id: 'evt-001', shipment_id: 'shipment-001', tracking_number: 'SBOX-12345',
    status: 'created', status_description: 'Shipment Created', location: 'Warehouse',
    event_time: NOW, provider_raw: {}, created_at: NOW,
  },
  {
    id: 'evt-002', shipment_id: 'shipment-001', tracking_number: 'SBOX-12345',
    status: 'in_transit', status_description: 'In Transit', location: 'Dhaka Hub',
    event_time: NOW, provider_raw: {}, created_at: NOW,
  },
];
  ...
  CourierGatewayService: {
    trackShipment: jest.fn().mockResolvedValue({
      ...
      data: {
        ...
        updates: [
          { status: 'delivered', statusDescription: 'Delivered', location: 'Recipient', timestamp: new Date().toISOString() },
        ],
      },
    }),
  },
...
describe('TrackingService', () => {
  ...
  it('should get tracking timeline by tracking number', async () => {
    const timeline = await service.getTrackingTimeline('SBOX-12345', 'tracking_number');
  ...
  it('should return null for unknown tracking number', async () => {
    ...
    const timeline = await s.getTrackingTimeline('UNKNOWN-999', 'tracking_number');
