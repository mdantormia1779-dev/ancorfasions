...
function makeRequest(body: object, url = 'http://localhost/api/webhooks/courier') {
...
describe('Provider-specific Webhook Route (/api/webhooks/courier/[provider])', () => {
  it('should return 200 for valid steadfast webhook', async () => {
    const req = makeRequest({ tracking_code: 'SBOX-TEST', status: 'Delivered' });
    const res = await providerWebhookPost(req, { params: Promise.resolve({ provider: 'sandbox' }) });
...
describe('Generic Webhook Route (/api/webhooks/courier?provider=)', () => {
  it('should return 400 if provider is missing', async () => {
    const req = makeRequest({ tracking_code: 'TEST' }, 'http://localhost/api/webhooks/courier');
