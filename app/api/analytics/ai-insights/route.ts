...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      summary:
        "Revenue is up 13.6% today driven primarily by strong performance in the Winter Collection.",
      risks: [
      ...
      recommendations: [
        {
          type: "Inventory",
          action:
            "Consider 10% discount on 'Classic White Tees'. Stock has aged > 90 days.",
        },
        {
          type: "Marketing",
          action:
            "Shift budget from Google Ads to Meta. Meta ROAS is outperforming by 22%.",
        },
      ],
    });
  } catch (error) {
