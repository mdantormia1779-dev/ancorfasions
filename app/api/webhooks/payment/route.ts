...
export async function POST(req: Request) {
  try {
    ...
    const result = await paymentWebhookService.processWebhook(provider, payload, headers);

    if (result.success) {
      // Enqueue the normalized event for async processing (e.g. updating order status)
      await WebhookService.enqueueIncomingWebhook('PAYMENT', provider, result.data);
