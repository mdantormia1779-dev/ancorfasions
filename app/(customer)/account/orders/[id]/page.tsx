...
export default async function OrderDetailsPage({ params }: { params: { id: string } }) {
  ...
  const shippingAddress = order.order_addresses?.find((a: any) => a.address_type === 'SHIPPING');
                ...
                {order.order_items?.map((item: any) => (
                  <div key={item.id} className="py-4 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Qty: {item.quantity} x {formatCurrency(item.unit_price)}</p>
                    </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {order.order_status_history?.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((history: any) => (
                  <div key={history.id} className="flex gap-4 items-start">
                    <div className="w-2 h-2 mt-2 bg-slate-400 rounded-full"></div>
                    <div>
                      <p className="font-medium">{history.status.replace(/_/g, ' ')}</p>
                      <p className="text-sm text-slate-500">{new Date(history.created_at).toLocaleString()}</p>
