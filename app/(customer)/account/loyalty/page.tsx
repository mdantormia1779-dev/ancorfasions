...
export default async function LoyaltyPage() {
  ...
  const getNextTier = (tier: string) => {
    switch (tier) {
      case "MEMBER":
        return { name: "SILVER", threshold: 1000 };
      case "SILVER":
        return { name: "GOLD", threshold: 5000 };
      case "GOLD":
        return { name: "PLATINUM", threshold: 10000 };
      case "PLATINUM":
        return { name: "VIP", threshold: 20000 };
      default:
        return { name: "SILVER", threshold: 1000 };
    }
  };
  ...
  const progress = Math.min(
    (loyalty.points / loyalty.nextTierPoints) * 100,
    100
  );
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              You are {loyalty.nextTierPoints - loyalty.points} points away from{" "}
              {loyalty.nextTier}.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Equals BDT {(loyalty.points * 0.5).toFixed(2)} in rewards
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-4">
            <div
              className={`rounded-lg border p-4 text-center ${loyalty.tier === "SILVER" ? "border-primary bg-muted" : "bg-muted/50"}`}
            >
            ...
            <div
              className={`rounded-lg border p-4 text-center ${loyalty.tier === "GOLD" ? "border-yellow-500 bg-yellow-500/10" : "border-yellow-500/20 bg-yellow-500/5"}`}
            >
              <p className="font-bold text-yellow-700">
                GOLD {loyalty.tier === "GOLD" ? "(Current)" : ""}
              </p>
            ...
            <div
              className={`rounded-lg border p-4 text-center ${loyalty.tier === "PLATINUM" ? "border-primary shadow-sm" : "opacity-70"}`}
            >
            ...
            <div
              className={`rounded-lg border p-4 text-center ${loyalty.tier === "VIP" ? "bg-primary text-primary-foreground" : "opacity-70"}`}
            >
