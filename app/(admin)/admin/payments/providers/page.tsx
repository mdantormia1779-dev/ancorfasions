...
export default async function PaymentProvidersPage() {
  ...
  const [{ data: bkashData }, { data: sslData }] = await Promise.all([
    supabase.from('settings').select('value').eq('key', 'payment_bkash').single(),
    supabase.from('settings').select('value').eq('key', 'payment_sslcommerz').single(),
  ]);

  const bkash = bkashData?.value
    ? { key: 'payment_bkash', enabled: bkashData.value.enabled ?? false, credentials: bkashData.value }
    : DEFAULT_CONFIG('payment_bkash');

  const ssl = sslData?.value
    ? { key: 'payment_sslcommerz', enabled: sslData.value.enabled ?? false, credentials: sslData.value }
