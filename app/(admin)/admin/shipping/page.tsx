...
import { Package, Truck, CheckCircle2, XCircle, AlertTriangle, RefreshCcw } from 'lucide-react';
...
export default function AdminShippingPage() {
  ...
  const [courierCode, setCourierCode] = useState<CourierProviderCode | undefined>();

  const { data, isLoading } = useShipments({ page, limit: 20, status, search: search || undefined, courierCode });
  ...
  const stats = {
    ...
    inTransit: shipments.filter((s: any) => ['in_transit', 'out_for_delivery', 'picked_up'].includes(s.status)).length,
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage courier assignments, labels, and tracking</p>
        </div>
        <div className="flex gap-2">
          <Link href="/admin/shipping/zones">
            <Button variant="outline" size="sm">Delivery Zones</Button>
          </Link>
          <Link href="/admin/shipping/providers">
            <Button variant="outline" size="sm">Courier Providers</Button>
          </Link>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            ...
            <select
              ...
              value={status ?? ''}
              onChange={(e) => { setStatus(e.target.value as ShipmentStatus || undefined); setPage(1); }}
            >
            ...
            <select
              className="border rounded-md px-3 py-2 text-sm"
              value={courierCode ?? ''}
              onChange={(e) => { setCourierCode(e.target.value as CourierProviderCode || undefined); setPage(1); }}
            >
        ...
        <CardContent>
          {isLoading ? (
            <div className="flex items-center justify-center py-12 text-muted-foreground">Loading shipments…</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {shipments.map((s: any) => (
                  <TableRow key={s.id}>
                    <TableCell className="font-medium">
                      <Link href={`/admin/shipping/${s.id}`} className="text-blue-600 hover:underline">
                        {s.shipment_number}
                      </Link>
                    </TableCell>
                    <TableCell>
                      <Link href={`/admin/orders/${s.order_id}`} className="text-blue-600 hover:underline text-xs">
                        {s.order_number || '—'}
                      </Link>
                    </TableCell>
                    <TableCell>
                      <span className="text-xs font-medium">
                        {s.courier_provider_code ? COURIER_LABELS[s.courier_provider_code] ?? s.courier_provider_code : '—'}
                      </span>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm">{s.recipient_name}</div>
                      <div className="text-xs text-muted-foreground">{s.recipient_city}</div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {s.is_cod ? (
                        <span className="text-xs font-medium text-orange-700 bg-orange-100 rounded px-1.5 py-0.5">
                    ...
                    <TableCell>
                      <span className={`text-xs font-medium rounded px-2 py-1 ${STATUS_COLORS[s.status as ShipmentStatus] ?? ''}`}>
