...
export class AnalyticsRepository {
  ...
  static async getExecutiveSummary(dateRange?: DateRange) {
    ...
    // Customers
    let customersQuery = supabase.from('customer_profiles').select('id, created_at, status');
    if (dateRange) {
    ...
    const activeCustomers = customers?.filter(c => c.status === 'active').length || totalCustomers; // Fallback to all if status not present
    
    const totalInventoryValue = (inventory?.reduce((sum, item) => sum + (item.quantity_available || 0), 0) || 0) * 50; // Approximating $50 per item if we don't map to product
    ...
    return {
      ...
      revenueHistory: rollups?.map(r => ({ name: r.date, total: r.total_revenue })) || []
    };
  }
  ...
  static async getSalesAnalytics(dateRange?: DateRange) {
    ...
    let rollupsQuery = supabase.from('bi_daily_revenue_rollup').select('*').order('date', { ascending: true });
    if (dateRange) {
      rollupsQuery = rollupsQuery.gte('date', dateRange.from.toISOString().split('T')[0])
                                 .lte('date', dateRange.to.toISOString().split('T')[0]);
    }
    const { data: rollups } = await rollupsQuery;
    
    // Fallback to orders if no rollups
    let ordersQuery = supabase.from('orders').select('total_amount, created_at, status, payment_method, shipping_method');
    if (dateRange) {
    ...
    let dailySales = rollups?.map(r => ({ date: r.date, revenue: r.total_revenue })) || [];
    ...
    if (dailySales.length === 0 && orders) {
      ...
      dailySales = Object.keys(salesByDate).map(date => ({ date, revenue: salesByDate[date] })).sort((a, b) => a.date.localeCompare(b.date));
    }
    ...
    orders?.forEach(order => {
      if (order.payment_method) {
        paymentMethods[order.payment_method] = (paymentMethods[order.payment_method] || 0) + (order.total_amount || 0);
      }
      if (order.shipping_method) {
        shippingMethods[order.shipping_method] = (shippingMethods[order.shipping_method] || 0) + (order.total_amount || 0);
      }
    });

    return {
      dailySales,
      paymentMethodSales: Object.keys(paymentMethods).map(k => ({ name: k, value: paymentMethods[k] })),
      shippingMethodSales: Object.keys(shippingMethods).map(k => ({ name: k, value: shippingMethods[k] })),
  ...
  static async getOrderAnalytics(dateRange?: DateRange) {
    const supabase = await createClient();
    let ordersQuery = supabase.from('orders').select('id, status, created_at, total_amount');
    ...
    const statusCounts: Record<string, number> = {
      pending: 0, processing: 0, packed: 0, shipped: 0, delivered: 0, returned: 0, cancelled: 0
    };
  ...
  static async getCustomerAnalytics(dateRange?: DateRange) {
    ...
    const { data: customers } = await customersQuery;
    const { data: orders } = await supabase.from('orders').select('customer_id, total_amount');
    ...
    const averageLTV = Object.values(customerSpend).length > 0 
      ? Object.values(customerSpend).reduce((a,b)=>a+b, 0) / Object.values(customerSpend).length 
      : 0;
  ...
  static async getProductAnalytics() {
    ...
    const { data: products } = await supabase.from('products').select('id, name, views');
    ...
    orderItems?.forEach(item => {
      if (item.product_id) {
         ...
         productSales[item.product_id].rev += ((item.quantity || 1) * (item.unit_price || 0));
      }
    });

    const salesArray = Object.keys(productSales).map(pid => {
    ...
    const mostViewed = [...(products || [])].sort((a, b) => (b.views || 0) - (a.views || 0)).slice(0, 10);
  ...
  static async getMarketingAnalytics(dateRange?: DateRange) {
    ...
    let couponsQuery = supabase.from('coupon_usages').select('discount_applied, created_at, code');
    if (dateRange) {
    ...
    const { data: customers } = await supabase.from('customer_profiles').select('accepts_marketing');
    
    const totalDiscountGiven = coupons?.reduce((acc, c) => acc + (c.discount_applied || 0), 0) || 0;
    
    const newsletterSubscribers = customers?.filter(c => c.accepts_marketing).length || 0;
    ...
    const topCoupons = Object.keys(couponUsageStats).map(code => ({
       code,
