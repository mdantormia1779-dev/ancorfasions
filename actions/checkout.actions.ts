...
import { CheckoutInformationFormValues, CheckoutPaymentFormValues, CheckoutShippingFormValues } from '@/schemas/checkout.schema';
...
export async function initializeCheckoutSessionAction(cartId: string, guestEmail?: string) {
  const userId = await getUserId();
  const checkoutService = new CheckoutService();
  return await checkoutService.initializeSession(cartId, userId, guestEmail);
}

export async function processInformationAction(sessionId: string, data: CheckoutInformationFormValues) {
  const checkoutService = new CheckoutService();
  await checkoutService.processInformationStep(sessionId, data);
  revalidatePath('/checkout');
}

export async function processShippingAction(sessionId: string, data: CheckoutShippingFormValues) {
  const checkoutService = new CheckoutService();
  await checkoutService.processShippingStep(sessionId, data);
  revalidatePath('/checkout');
}

export async function processPaymentAction(sessionId: string, data: CheckoutPaymentFormValues) {
  const checkoutService = new CheckoutService();
  const session = await checkoutService.getSession(sessionId);
  if (!session) throw new Error('Session not found');

  await checkoutService.processPaymentStep(sessionId, data, session.shipping_address_snapshot);
  revalidatePath('/checkout');
}
...
export async function processCheckoutAction(cartId: string, sessionId: string, data: any) {
  const checkoutService = new CheckoutService();
  const userId = await getUserId();
  ...
  try {
    ...
    // 4. Process Payment
    const session = await checkoutService.getSession(sessionId);
    await checkoutService.processPaymentStep(sessionId, data.payment, session?.shipping_address_snapshot);
