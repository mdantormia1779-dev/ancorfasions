...
export class PaymentService {
  ...
  async initializePayment(request: PaymentInitializeRequest): Promise<PaymentInitializeResponse> {
    try {
      // 1. Get Provider
      const provider = await paymentFactory.getActiveProvider(request.providerCode)
        .catch(async () => await paymentFactory.getFallbackProvider());

      const providerEntity = await paymentRepository.getProviderByCode(provider.getCode());
   ...
   */
  async verifyPayment(sessionId: string, payload: Record<string, any>): Promise<PaymentTransaction> {
    const session = await paymentRepository.getSessionById(sessionId);
    if (!session) throw new Error('Payment session not found');

    const transaction = await paymentRepository.getTransactionBySessionId(sessionId);
    if (!transaction) throw new Error('Payment transaction not found');

    const providerEntity = await paymentRepository.getProviderByCode(session.provider_id || '');
    ...
    // Update Transaction
    const updatedTransaction = await paymentRepository.updateTransaction(transaction.id, {
      ...verifiedData
    });

    // Update Session
    const sessionStatus = updatedTransaction.status === 'completed' ? 'completed' : 'failed';
    await paymentRepository.updateSession(session.id, { status: sessionStatus });
