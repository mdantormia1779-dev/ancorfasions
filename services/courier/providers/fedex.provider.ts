...
export class FedExProvider implements ICourierProvider {
  ...
  private async authHeaders(): Promise<Record<string, string>> {
    ...
    return { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, 'X-locale': 'en_US' };
  }
  ...
  async createConsignment(request: ConsignmentRequest): Promise<ProviderResponse<ConsignmentResponse>> {
    try {
      ...
      const payload = {
        ...
        requestedShipment: {
          shipper: {
            contact: { companyName: 'Anchor Fashion', phoneNumber: '+8801700000000' },
            address: { streetLines: ['House 12, Dhanmondi'], city: 'Dhaka', countryCode: 'BD' },
          },
          recipients: [{
            contact: { personName: request.recipientName, phoneNumber: request.recipientPhone },
            address: { streetLines: [request.recipientAddress], city: request.recipientCity || 'Dhaka', countryCode: 'BD' },
          }],
      ...
      if (shipmentDetail?.masterTrackingNumber) {
        return {
          ...
          data: {
            ...
            labelUrl: shipmentDetail.pieceResponses?.[0]?.packageDocuments?.[0]?.url,
            status: 'created',
          },
        };
      }

      return this.errorResponse('API_ERROR', data.errors?.[0]?.message || 'FedEx shipment creation failed');
    } catch (err: any) {
  ...
  async cancelConsignment(consignmentId: string): Promise<ProviderResponse<void>> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/ship/v1/shipments/cancel`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ accountNumber: { value: this.accountNumber }, trackingNumber: consignmentId }),
      });
      ...
      const data = await response.json();
      return this.errorResponse('API_ERROR', data.errors?.[0]?.message || 'Cancel failed');
    } catch (err: any) {
      return this.errorResponse('EXCEPTION', err.message);
    }
  }
  ...
  async trackShipment(trackingCode: string): Promise<ProviderResponse<TrackingResult>> {
    try {
      ...
      if (track) {
        const updates: TrackingUpdate[] = (track.scanEvents || []).map((e: any) => ({
          status: e.eventType,
  ...
  async generateLabel(consignmentId: string): Promise<ProviderResponse<{ labelUrl: string }>> {
    return {
      ...
      providerId: this.id,
      timestamp: new Date().toISOString(),
      data: { labelUrl: `${this.baseUrl}/ship/v1/shipments/${consignmentId}/documents` },
    };
  }
