...
export class SteadfastProvider implements ICourierProvider {
  ...
  async createConsignment(request: ConsignmentRequest): Promise<ProviderResponse<ConsignmentResponse>> {
    try {
      ...
      return this.errorResponse('API_ERROR', data.message || 'Failed to create consignment');
    } catch (err: any) {
  ...
  async cancelConsignment(consignmentId: string): Promise<ProviderResponse<void>> {
    try {
      const response = await fetch(`${this.baseUrl}/delete_order/${consignmentId}`, {
        method: 'DELETE',
        headers: this.headers,
      });
      ...
      return this.errorResponse('API_ERROR', data.message || 'Failed to cancel consignment');
    } catch (err: any) {
      return this.errorResponse('EXCEPTION', err.message);
    }
  }
  ...
  async trackShipment(trackingCode: string): Promise<ProviderResponse<TrackingResult>> {
    try {
      ...
      if (data.status === 200) {
        const updates: TrackingUpdate[] = (data.activity_log || []).map((log: any) => ({
          status: log.status,
      ...
      return this.errorResponse('API_ERROR', 'Failed to retrieve tracking info');
    } catch (err: any) {
      return this.errorResponse('EXCEPTION', err.message);
    }
  }
  ...
  async generateLabel(consignmentId: string): Promise<ProviderResponse<{ labelUrl: string }>> {
    ...
    return {
      ...
      providerId: this.id,
      timestamp: new Date().toISOString(),
      data: { labelUrl: `https://portal.packzy.com/print-label/${consignmentId}` },
    };
  }
  ...
  async processWebhook(payload: unknown, signature: string): Promise<ProviderResponse<NormalizedWebhookEvent>> {
    try {
      ...
      const normalized = this.normalizeStatus(body.status || body.delivery_status || '');
