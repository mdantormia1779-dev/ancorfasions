...
export class SundarbanProvider implements ICourierProvider {
      ...
      if (data.success && data.data?.tracking_number) {
        return {
          ...
          data: {
            ...
            consignmentId: String(data.data.consignment_id || data.data.tracking_number),
            status: 'created',
          },
        };
      }

      return this.errorResponse('API_ERROR', data.message || 'Failed to create Sundarban order');
    } catch (err: any) {
  ...
  async cancelConsignment(consignmentId: string): Promise<ProviderResponse<void>> {
    return {
      ...
      providerId: this.id,
      timestamp: new Date().toISOString(),
      error: { code: 'NOT_SUPPORTED', message: 'Sundarban requires manual cancellation via portal.' },
    };
  }

  async trackShipment(trackingCode: string): Promise<ProviderResponse<TrackingResult>> {
    try {
      const response = await fetch(`${this.baseUrl}/order/track/${trackingCode}`, {
        method: 'GET',
        headers: this.headers,
      });
      ...
      if (data.success && data.data) {
        const updates: TrackingUpdate[] = (data.data.events || []).map((e: any) => ({
          status: e.status,
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          consignmentId: String(body.consignment_id || body.tracking_number || ''),
