...
export class PaperflyProvider implements ICourierProvider {
  ...
  async createConsignment(request: ConsignmentRequest): Promise<ProviderResponse<ConsignmentResponse>> {
    try {
      ...
      return this.errorResponse('API_ERROR', data.message || 'Failed to create Paperfly order');
    } catch (err: any) {
  ...
  async trackShipment(trackingCode: string): Promise<ProviderResponse<TrackingResult>> {
    try {
      const response = await fetch(`${this.baseUrl}/order/track?tracking_id=${trackingCode}`, {
        method: 'GET',
        headers: this.headers,
      });
      ...
      if (data.success && data.tracking_info) {
        const updates: TrackingUpdate[] = (data.tracking_info.logs || []).map((log: any) => ({
          status: log.status,
        ...
        return {
          ...
          timestamp: new Date().toISOString(),
          data: {
            status: this.normalizeStatus(data.tracking_info.current_status || ''),
