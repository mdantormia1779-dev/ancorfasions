...
export class DHLProvider implements ICourierProvider {
  ...
  async createConsignment(request: ConsignmentRequest): Promise<ProviderResponse<ConsignmentResponse>> {
    try {
      ...
      const payload = {
        plannedShippingDateAndTime: shipDate.toISOString().replace(/\.\d{3}Z/, ' GMT+0600'),
        ...
        customerDetails: {
          shipperDetails: {
            postalAddress: {
              postalCode: '1213', cityName: 'Dhaka', countryCode: 'BD',
              addressLine1: 'House 12, Road 5, Dhanmondi',
            },
            contactInformation: { fullName: 'Anchor Fashion', phone: '+8801700000000', email: 'logistics@anchorfashion.com' },
          },
          receiverDetails: {
            postalAddress: {
            ...
            contactInformation: { fullName: request.recipientName, phone: request.recipientPhone },
          },
        },
        content: {
          packages: [{ weight: request.weight ?? 0.5, dimensions: { length: 20, width: 15, height: 5 } }],
      ...
      return this.errorResponse('API_ERROR', data.title || 'Failed to create DHL shipment');
    } catch (err: any) {
  ...
  async cancelConsignment(consignmentId: string): Promise<ProviderResponse<void>> {
    try {
      const response = await fetch(`${this.baseUrl}/shipments/${consignmentId}/cancel`, {
        method: 'DELETE',
        headers: this.headers,
      });
  ...
  async trackShipment(trackingCode: string): Promise<ProviderResponse<TrackingResult>> {
    try {
      ...
      if (shipment) {
        const updates: TrackingUpdate[] = (shipment.events || []).map((e: any) => ({
          status: e.typeCode,
  ...
  async generateLabel(consignmentId: string): Promise<ProviderResponse<{ labelUrl: string }>> {
    return {
      ...
      providerId: this.id,
      timestamp: new Date().toISOString(),
      data: { labelUrl: `https://express.api.dhl.com/mydhlapi/shipments/${consignmentId}/documents` },
    };
  }
