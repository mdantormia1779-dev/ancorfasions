...
export class RedXProvider implements ICourierProvider {
  ...
  async createConsignment(request: ConsignmentRequest): Promise<ProviderResponse<ConsignmentResponse>> {
    try {
      const payload = {
        ...
        delivery_area: request.recipientDistrict || request.recipientCity || 'Dhaka',
      ...
      return this.errorResponse('API_ERROR', data.message || 'Failed to create RedX parcel');
    } catch (err: any) {
  ...
  async cancelConsignment(consignmentId: string): Promise<ProviderResponse<void>> {
    try {
      const response = await fetch(`${this.baseUrl}/parcel/${consignmentId}/cancel`, {
        method: 'PATCH',
        headers: this.headers,
      });
      ...
      return this.errorResponse('API_ERROR', data.message || 'Failed to cancel RedX parcel');
    } catch (err: any) {
      return this.errorResponse('EXCEPTION', err.message);
    }
  }
  ...
  async trackShipment(trackingCode: string): Promise<ProviderResponse<TrackingResult>> {
    try {
      const response = await fetch(`${this.baseUrl}/parcel/track?tracking_id=${trackingCode}`, {
        method: 'GET',
        headers: this.headers,
      });
      ...
      if (data.tracking_events) {
        const updates: TrackingUpdate[] = data.tracking_events.map((event: any) => ({
          status: event.type,
