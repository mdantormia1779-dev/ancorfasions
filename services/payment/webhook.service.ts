...
export class WebhookService {
   ...
   */
  async handleWebhook(providerCode: PaymentProviderCode, payload: any, headers: Record<string, any>, rawBody: string) {
    const providerConfig = await this.providerRepo.getProviderByCode(providerCode);
    if (!providerConfig) {
    ...
    try {
      ...
      // 3. Mark as completed
      await this.paymentRepo.updateWebhookEvent(webhookRecord.id, { status: 'completed' });

      return { success: true };
    } catch (error: any) {
      // 4. Mark as failed for retry mechanism
      await this.paymentRepo.updateWebhookEvent(webhookRecord.id, { status: 'failed' } as any);
      throw error;
    }
  }
