...
export class StripeProvider implements PaymentProvider {
  ...
  async initiatePayment(request: PaymentIntentRequest): Promise<IntegrationResponse<PaymentIntentResponse>> {
    try {
  ...
  async verifyPayment(request: PaymentVerifyRequest): Promise<IntegrationResponse<{ status: string; amount: number; currency: string }>> {
    try {
      const response = await fetch(`${this.getBaseUrl()}/payment_intents/${request.transactionId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${this.secretKey}`
        }
  ...
  async processWebhook(payload: any, signature: string): Promise<IntegrationResponse<any>> {
