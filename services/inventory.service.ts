...
export class InventoryService {
  ...
  async transferStock(variantId: string, fromWarehouseId: string, toWarehouseId: string, quantity: number, reason: string, notes?: string): Promise<void> {
    await this.repository.transferStock(variantId, fromWarehouseId, toWarehouseId, quantity, reason, notes);
   ...
   */
  async getMovements(page: number = 1, limit: number = 20, warehouseId?: string, variantId?: string) {
    return await this.repository.getMovements(page, limit, warehouseId, variantId);
  }

  /**
   * Check stock levels and create alerts if needed
   */
  async checkStockAlerts(variantId: string, warehouseId: string): Promise<void> {
    const levels = await this.repository.getStockLevel(variantId, warehouseId);
    ...
    if (alertType) {
      ...
      const { data: existing } = await supabase
        ...
        .eq('status', 'ACTIVE')
        .contains('metadata', { variant_id: variantId, warehouse_id: warehouseId })
        .limit(1)
        .maybeSingle();
      ...
      if (!existing) {
        await supabase.from('system_alerts').insert({
          ...
          status: 'ACTIVE',
          metadata: { variant_id: variantId, warehouse_id: warehouseId, current_stock: level.quantity_available, reorder_point: level.reorder_point }
        });
      }
    }
  }
