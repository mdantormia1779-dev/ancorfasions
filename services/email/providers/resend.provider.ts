...
export class ResendProvider implements EmailProvider {
  ...
  constructor(config: Record<string, any>) {
    ...
    this.defaultFrom = config.defaultFrom || process.env.RESEND_DEFAULT_FROM || 'noreply@anchorfashion.com';
  }

  async sendEmail(request: EmailSendRequest): Promise<IntegrationResponse<{ messageId: string }>> {
    try {
      const payload: any = {
      ...
      if (request.cc) payload.cc = Array.isArray(request.cc) ? request.cc : [request.cc];
      if (request.bcc) payload.bcc = Array.isArray(request.bcc) ? request.bcc : [request.bcc];
