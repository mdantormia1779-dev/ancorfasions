...
export async function generateSalesForecast(historicalData: any[]): Promise<{ forecast?: any; error?: string }> {
  try {
    ...
    const parsedData = jsonMatch ? JSON.parse(jsonMatch[0]) : { rawResponse: text };
...
export async function generateInventoryInsights(inventoryData: any[]): Promise<{ insights?: any; error?: string }> {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-pro' });
...
export async function saveAIPrediction(data: unknown): Promise<{ success?: boolean; error?: string }> {
  try {
