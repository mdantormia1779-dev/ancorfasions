...
export class WebhookService {
   ...
   */
  static async enqueueIncomingWebhook(eventType: string, provider: string, payload: any): Promise<boolean> {
   ...
   */
  static async triggerOutgoingWebhook(webhookId: string, eventType: string, payload: any): Promise<boolean> {
    try {
      ...
      if (error) {
        console.error('[WebhookService] Failed to queue outgoing webhook:', error);
        return false;
      }

      return true;
    } catch (error) {
      console.error('[WebhookService] Exception queueing outgoing webhook:', error);
      return false;
    }
  }
}
