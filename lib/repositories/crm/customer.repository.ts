...
export class CustomerRepository {
  ...
  async getCustomerSegments() {
    ...
    const { data, error } = await supabase
      .rpc('get_customer_segments_aggregation');
    ...
    if (error || !data) {
      ...
      const stages: CustomerLifecycleStage[] = ['PROSPECT', 'FIRST_TIME_BUYER', 'REPEAT_CUSTOMER', 'LOYAL', 'AT_RISK', 'CHURNED'];
  ...
  async getLoyaltyStats() {
    ...
    const counts: Record<string, number> = {
      'SILVER': 0, 'GOLD': 0, 'PLATINUM': 0, 'VIP': 0
    };
    ...
    return [
      ...
      },
      {
        ...
        benefits: ['Priority support', 'Free shipping', '10% discount on birthdays'],
      ...
      },
      {
        ...
        benefits: ['Dedicated manager', 'Free express shipping', 'Early access to sales'],
      ...
      },
      {
        ...
        benefits: ['All Platinum benefits', 'Exclusive event invites', 'Personal stylist'],
  ...
  async getCRMSummary() {
    ...
    return {
      ...
      avgLifetimeValue: 1284.50, // Usually calculated from orders
