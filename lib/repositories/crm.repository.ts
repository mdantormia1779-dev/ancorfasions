...
export class CRMRepository {
  ...
  async getNotes(profileId: string): Promise<CRMNote[]> {
    const supabase = await createClient();
    const { data, error } = await supabase.from('crm_notes').select('*').eq('profile_id', profileId).order('created_at', { ascending: false });
    if (error) throw error;
  ...
  async getCommunicationLogs(profileId: string): Promise<CommunicationLog[]> {
    const supabase = await createClient();
    const { data, error } = await supabase.from('communication_logs').select('*').eq('profile_id', profileId).order('created_at', { ascending: false });
    if (error) throw error;
