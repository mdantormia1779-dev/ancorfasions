...
import { CheckoutFormValues, checkoutFormSchema } from '@/schemas/checkout.schema';
...
export async function fetchCheckoutSessionAction(cartId: string) {
  try {
    ...
    const session = await CheckoutService.getOrInitializeSession(cartId, userId, guestEmail);
...
export async function updateCheckoutStepAction(sessionId: string, step: CheckoutStep, data: any) {
  try {
...
export async function processCheckoutAction(cartId: string, checkoutSessionId: string, formData: CheckoutFormValues) {
  try {
    // Validate form data
