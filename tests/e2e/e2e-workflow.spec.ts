...
test.describe('Enterprise Critical Path (Smoke Test)', () => {
  test('User can browse, add to cart, and initiate checkout', async ({ page, baseURL }) => {
    // 1. Go to homepage
    await page.goto('/');
    ...
    if (await products.count() > 0) {
        ...
        if (await addToCartBtn.isVisible()) {
            ...
            const cartLink = page.getByRole('link', { name: /Cart/i, exact: false }).first();
  ...
  test('Admin dashboard is secure and redirects unauthorized users', async ({ page }) => {
