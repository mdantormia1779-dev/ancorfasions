...
describe('PaymentWebhookService', () => {
  ...
  it('should process incoming webhook successfully', async () => {
    ...
    vi.mocked(paymentRepository.createWebhookEvent).mockResolvedValue({ id: 'webhook-1', retry_count: 0 } as any);
    ...
    const result = await paymentWebhookService.processWebhook('mock', { type: 'payment_success' }, {});

    expect(result.success).toBe(true);
    expect(paymentRepository.createWebhookEvent).toHaveBeenCalled();
    expect(mockProvider.processWebhook).toHaveBeenCalledWith({ type: 'payment_success' }, {});
    expect(paymentRepository.updateWebhookEvent).toHaveBeenCalledWith('webhook-1', expect.objectContaining({ status: 'completed' }));
  });
  ...
  it('should handle webhook processing failure', async () => {
    ...
    await expect(paymentWebhookService.processWebhook('mock', { type: 'payment_success' }, {}))
      .rejects
      .toThrow('Invalid signature');

    expect(paymentRepository.updateWebhookEvent).toHaveBeenCalledWith('webhook-1', expect.objectContaining({ 
      status: 'failed', 
