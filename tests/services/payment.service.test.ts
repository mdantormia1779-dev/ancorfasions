...
describe("PaymentService", () => {
  ...
  describe("initializePayment", () => {
    it("should initialize payment via provider and create db records", async () => {
      ...
      vi.mocked(paymentRepository.createSession).mockResolvedValue({
        id: "session-1",
      } as any);
      vi.mocked(paymentRepository.createTransaction).mockResolvedValue({
        id: "txn-1",
      } as any);
      ...
      const mockProvider = {
        ...
        initializePayment: vi
          .fn()
          .mockResolvedValue({ gatewayUrl: "http://mock.gateway/pay" }),
      };
      vi.spyOn(paymentFactory, "getActiveProvider").mockResolvedValue(
        mockProvider as any
      );
      ...
      expect(paymentRepository.updateSession).toHaveBeenCalledWith(
        "session-1",
  ...
  describe("verifyPayment", () => {
    it("should verify payment and update transaction status to completed", async () => {
      vi.mocked(paymentRepository.getSessionById).mockResolvedValue({
        id: "session-1",
        provider_id: "provider-1",
      } as any);
      vi.mocked(paymentRepository.getTransactionBySessionId).mockResolvedValue({
        id: "txn-1",
      } as any);
      vi.mocked(paymentRepository.getProviderByCode).mockResolvedValue({
        id: "provider-1",
      ...
      vi.mocked(paymentRepository.updateTransaction).mockResolvedValue({
        id: "txn-1",
        status: "completed",
      } as any);
      ...
      const result = await paymentService.verifyPayment("session-1", {
        status: "success",
      });

      expect(result.status).toBe("completed");
      expect(paymentRepository.updateTransaction).toHaveBeenCalledWith(
        "txn-1",
        { status: "completed" }
      );
      expect(paymentRepository.updateSession).toHaveBeenCalledWith(
