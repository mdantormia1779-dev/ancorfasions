...
vi.mock('@/repositories/seo.repository', () => {
  return {
    SEORepository: vi.fn().mockImplementation(() => {
      return {
        getMetadataByEntity: vi.fn().mockResolvedValue({ id: '1', title: 'SEO Title', entity_type: 'page' }),
        upsertMetadata: vi.fn().mockResolvedValue({ id: '1', title: 'New Title', entity_type: 'page' }),
      };
    }),
  };
});
