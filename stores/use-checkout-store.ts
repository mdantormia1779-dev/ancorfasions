...
import { fetchCheckoutSessionAction, updateCheckoutStepAction } from '@/lib/actions/checkout.actions';
import { CheckoutFormValues, CheckoutInformationFormValues, CheckoutShippingFormValues, CheckoutPaymentFormValues } from '@/schemas/checkout.schema';
...
export const useCheckoutStore = create<CheckoutState>((set, get) => ({
      ...
      set({ 
        ...
        formData: {
          information: {
            ...
            shipping_address: res.session.shipping_address_snapshot || undefined,
  ...
  updateStep: async (step: CheckoutStep) => {
    ...
    if (step === 'SHIPPING' || step === 'PAYMENT' || step === 'REVIEW') {
      if (formData.information) {
        ...
        dataToSync.shipping_address_snapshot = formData.information.shipping_address;
      }
    }
    ...
    if (step === 'REVIEW') {
      if (formData.payment) {
        ...
        if (!formData.payment.billing_address_same_as_shipping && formData.payment.billing_address) {
          dataToSync.billing_address_snapshot = formData.payment.billing_address;
        } else if (formData.information?.shipping_address) {
          dataToSync.billing_address_snapshot = formData.information.shipping_address;
        }
      }
    }
  ...
  setInformation: (data) => set((state) => ({ formData: { ...state.formData, information: data } })),
  setShipping: (data) => set((state) => ({ formData: { ...state.formData, shipping: data } })),
  setPayment: (data) => set((state) => ({ formData: { ...state.formData, payment: data } })),
  setNotes: (notes) => set((state) => ({ formData: { ...state.formData, notes } })),
