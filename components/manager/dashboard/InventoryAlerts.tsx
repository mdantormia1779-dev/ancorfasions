        ...
        {alerts.map((item) => (
          <div key={item.id} className="flex items-center justify-between space-x-4">
            <div className="flex items-center space-x-4">
              <div className={`p-2 rounded-full ${item.status === 'out' ? 'bg-destructive/10 text-destructive' : 'bg-orange-500/10 text-orange-500'}`}>
                {item.status === 'out' ? <PackageX className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}
              </div>
              <div>
            ...
            <div className="text-right">
              <Badge variant={item.status === 'out' ? "destructive" : "secondary"} className={item.status === 'low' ? 'bg-orange-500 hover:bg-orange-600 text-white' : ''}>
                {item.stock} in stock
              </Badge>
            </div>
          </div>
