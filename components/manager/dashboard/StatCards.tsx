...
interface StatCardsProps {
  kpis?: {
    ...
    processing: { value: number; trend: { value: number; isPositive: boolean } };
    alerts: { value: number };
  };
}
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {data.revenue.trend.isPositive ? '+' : '-'}{data.revenue.trend.value}% from last month
          </p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {data.orders.trend.isPositive ? '+' : '-'}{data.orders.trend.value}% from last month
          </p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {data.processing.trend.isPositive ? '+' : '-'}{data.processing.trend.value}% from last month
          </p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-destructive">{data.alerts.value}</div>
