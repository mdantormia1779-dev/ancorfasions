...
export function SecuritySettings({ loginHistory, activeSessions }: { loginHistory: any[], activeSessions: any[] }) {
          ...
          {activeSessions.length === 0 ? (
            ...
            <div className="space-y-4">
              {activeSessions.map((session, i) => (
                <div key={session.id || i} className="flex items-center justify-between py-2 border-b last:border-0">
                  <div className="flex items-center gap-3">
                    <Monitor className="w-6 h-6 text-muted-foreground" />
                    <div>
                      <p className="text-sm font-medium">{session.user_agent || 'Unknown Device'}</p>
                      <p className="text-xs text-muted-foreground">{session.ip_address} • Last active {new Date(session.last_active_at || session.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                  <Badge variant="outline" className="text-green-600 bg-green-50 border-green-200">Current Session</Badge>
                </div>
              ))}
            </div>
          )}
                ...
                {loginHistory.map((history, i) => (
                  <TableRow key={history.id || i}>
                    <TableCell className="text-sm">
                      {new Date(history.created_at || history.attempted_at).toLocaleString()}
                    </TableCell>
                    <TableCell className="text-sm">
                      {history.location || history.ip_address || 'Unknown'}
                    </TableCell>
                    <TableCell className="text-sm max-w-[200px] truncate" title={history.user_agent}>
                      {history.user_agent || 'Unknown Device'}
                    </TableCell>
                    <TableCell>
                      <Badge variant={history.status === 'SUCCESS' ? 'default' : 'destructive'} className={history.status === 'SUCCESS' ? 'bg-green-600 hover:bg-green-700' : ''}>
                        {history.status}
                      </Badge>
                    </TableCell>
                  </TableRow>
